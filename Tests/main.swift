...
check("и говорит, сколько скрыл",
      capped.rows.last?.contains(L("и ещё 8", "8 more")) ?? false)
...
// подозрительна». 17.09.2026 он показал свою цену - climits выводил 4
// живые сессии из 9, а четыре серверных работали от 25 до 120 часов.
// Сессии Remote Control живут неделями, и порог резал самое рабочее.
let stat = "1897108 (claude.exe) S 1 1897108 1897108 0 -1 4194560 12345 0 0 0 100 50 0 0 20 0 15 0 153826220 123456 789"
check("starttime берётся из /proc/<pid>/stat", Sessions.parseProcStat(stat) ?? "", "153826220")
// Имя процесса бывает со скобками и пробелами - считать поля с начала
// строки нельзя, только после последней закрывающей скобки.
let tricky = "42 ((sd-pam) weird) S 1 42 42 0 -1 4194560 1 0 0 0 1 1 0 0 20 0 1 0 999111 1 2"
check("имя со скобками не сдвигает поля", Sessions.parseProcStat(tricky) ?? "", "999111")
check("мусор вместо stat - это nil, а не выдуманное число",
      Sessions.parseProcStat("совсем не stat") == nil)

// Сомнение решается в пользу показа: спрятанная живая сессия - это работа,
// о которой человек не знает, и именно она и случилась.
check("нет токена в файле - процесс считаем своим",
      Sessions.sameProcess(pid: Int(getpid()), token: nil))
check("пустой токен - то же самое",
      Sessions.sameProcess(pid: Int(getpid()), token: "   "))
// Свой процесс сам себя опознаёт: токен берётся той же дорогой, что и
// сверяется, и на живом pid обязан сойтись.
let ownToken = Sessions.startToken(pid: Int(getpid()))
check("свой процесс опознаётся по токену старта", ownToken != nil)
if let t = ownToken {
    check("и сверка с ним проходит", Sessions.sameProcess(pid: Int(getpid()), token: t))
    check("а чужой токен её не проходит",
          !Sessions.sameProcess(pid: Int(getpid()), token: "1"))
}

// ---- заголовок группы машины -----------------------------------------------
//
// Поймано снимком 1.15.0: полная форма нагрузки («ОЗУ занято 9,8 из 16,0 ГБ
// · своп 1,1 из 6,0 ГБ · ЦП 1,2 из 8») в строку меню не влезает, и у всех
// трёх машин срезалась загрузка процессора. Резать её нельзя: она отвечает
// на другой вопрос, чем память, - машина может стоять колом при свободной
// памяти.
let macLoad = MachineMemory(totalMB: 16384, availableMB: 6349,
                            swapTotalMB: 6144, swapUsedMB: 1126,
                            load1: 1.2, cores: 8)
let macHead = "  " + L("мак", "mac") + " \u{00B7} " + macLoad.shortText
check("заголовок машины влезает в строку целиком", macHead.count <= Sessions.maxLine)
check("и процессор в нём уцелел", macHead.contains(L("ЦП", "CPU")))
check("и своп тоже", macHead.contains(L("своп", "swap")))
// Самый широкий случай: сервер с двузначными гигабайтами и длинным именем.
let wide = "  " + "vps7" + " \u{00B7} " + MachineMemory(totalMB: 131072, availableMB: 65536,
                                                   swapTotalMB: 65536, swapUsedMB: 32768,
                                                   load1: 12.5, cores: 64).shortText
check("и на трёхзначных числах тоже", wide.count <= Sessions.maxLine)
...
// Память машины: сколько занято и сколько осталось. Без второго числа
// сумма по сессиям не с чем сравнить - 1 ГБ на сервере с 4 ГБ и на маке
// с 32 это разные новости.
var mm = MachineMemory(totalMB: 3915, availableMB: 1224,
                       swapTotalMB: 7030, swapUsedMB: 1743)
check("занято считается как всего минус доступно",
      mm.text.contains(L("занято 2,6 из 3,8", "2.6 of 3.8")))
check("своп показан занятым из всего", mm.text.contains(L("1,7 из 6,9", "1.7 of 6.9")))
// На macOS свободного ОЗУ нет намеренно: система держит почти всю память
// под кэшем, и «свободно 300 МБ» там означает исправную работу, а не беду.
mm.availableMB = 0
check("без доступного показываем только всего",
      mm.text.contains(L("всего 3,8", "3.8 GB total")))
...
// Двойной пробел ловится сверкой с питоном удалённой стороны: там
// схлопывается всё пробельное сразу, и разойдись мы здесь - одна сессия
// подписывалась бы по-разному с разных машин.
check("двойной пробел внутри схлопывается так же, как перевод строки",
      Activity.lastPrompt(promptLine("да  1,2 и  3")), "да 1,2 и 3")
