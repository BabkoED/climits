...
final class MenuBarController: NSObject, NSMenuDelegate {
    ...
    func showMenuForShot() {
        ...
        // Счётчик машин в подписи под деньгами. В фикстуре он оставался
        // единицей, и кадр 1.14.0 показал «из 1 машин: этой и vps7, mono» -
        // подпись, невозможную в жизни. Живой код считает верно (единица
        // плюс ответившие), врал именно снимок; а снимок для того и снят,
        // чтобы показывать то, что увидит человек.
        machines = 1 + Set(remoteSessions.map { $0.machine }).count
