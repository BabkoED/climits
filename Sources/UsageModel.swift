...
// Откуда взялось число. Приём из codenotch (`Fidelity` у провайдера),
// и он ложится на то же правило, по которому мы отвечаем словами:
// у любого числа видна опора, иначе оценка читается как факт.
//
// Разница здесь не косметическая. Процент лимита считает тот, кто лимит
// и держит: спорить с ним нам нечем. Деньги мы считаем сами, своим прайсом,
// по расшифровкам тех машин, до которых достаём, - у этого числа две
// независимые причины разойтись с правдой, и обе наши.
//
// Что это меняет на экране: почти ничего, и это намеренно. Знак «≈» у денег
// стоял и раньше - но тремя разными литералами в трёх файлах, и добавить
// четвёртое место, забыв знак, было делом одной правки. Теперь правило
// одно. Видимая польза - в `--doctor`: когда число выглядит неверным, из
// отчёта видно, наше оно или их, и спрашивать надо в разные адреса.
enum Fidelity {
    case official     // сказал тот, кто держит лимит
    case derived      // посчитали мы: свой прайс, свои файлы

    // Знак при числе. У официального пусто: помечать нечего, это и есть
    // ответ. Приписка «точно» на нём была бы обещанием, которого никто
    // не давал.
    var mark: String {
        switch self {
        case .official: return ""
        case .derived:  return "\u{2248}"
        }
    }

    var word: String {
        switch self {
        case .official: return L("от Anthropic", "from Anthropic")
        case .derived:  return L("наш счёт", "our own count")
        }
    }

    // Число со знаком опоры. Единственное место, где этот знак ставится.
