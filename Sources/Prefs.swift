...
struct Prefs {
    ...
    // невозможен.
    //
    // Как назвать СВОЮ машину в заголовке её группы. Пусто - берётся имя
    // хоста до первой точки. Ярлык здесь по той же причине, что и у
    // удалённых: «Babko-Pro» - это то, как машина зовётся сама, а «мак» -
    // то, как её зовёт человек, и в списке из трёх машин важнее второе.
    static var localLabel: String {
        get { str("localLabel", "") }
        set { d.set(newValue, forKey: "localLabel") } }
    ...
    // Второй провайдер - ВЫКЛЮЧЕН по умолчанию, включается галочкой.
    //
    // В 1.20.0 он включался сам, если Codex стоит на машине. Антон
    // 24.09.2026, увидев раздел у себя: «интересно, но особо не нужен,
    // давай это включаемой функцией». Стоящий Codex ещё не значит, что
    // его лимиты нужны в меню.
    static var codexEnabled: Bool {
        get { bool("codexEnabled", false) } set { d.set(newValue, forKey: "codexEnabled") } }

    // Вид выпадающего меню: карточки (1.20.0, как у CodexBar) или прежние
