...
struct Prefs {
    ...
    // Второй путь обновления - Sparkle. ВЫКЛЮЧЕН по умолчанию, и это не
    // осторожность ради осторожности: обновление выполняет та версия,
    // которая уже стоит, и отказ нового пути запирает самообновление
    // насовсем. Пока галочка снята, выпуск со Sparkle не меняет ничего.
    // Дойдёт ли он до конца установки на неподписанном приложении -
    // проверяется только на живом Маке, здесь этого прогона нет.
    static var sparkleEnabled: Bool {
        get { bool("sparkleEnabled", false) } set { d.set(newValue, forKey: "sparkleEnabled") } }
