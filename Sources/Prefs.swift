...
struct Prefs {
    ...
    // Разбивка денег по проектам за то же окно, что и сами деньги.
    static var showProjects: Bool {
        get { bool("showProjects", false) } set { d.set(newValue, forKey: "showProjects") } }

    // Сколько проектов показываем. Ниже хвост всё равно не читают, а
    // меню растёт вниз без предела.
