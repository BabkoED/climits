...
enum Fmt {
    // Мегабайты человеку: «340 МБ», «7,1 ГБ».
    //
    // Порог в гигабайт, а не в тысячу: «1712 МБ» читается не порядком
    // величины, а цифрами, и вопрос «это много?» остаётся без ответа.
    // Запятая в русской локали - дробный разделитель, и «7.1» там
    // выглядит чужим числом.
