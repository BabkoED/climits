...
struct MachineMemory: Equatable {
    ...
    // ЗАГРУЗКА ПРОЦЕССОРА той машины: среднее за минуту и число ядер.
    //
    // Память отвечает на «влезет ли ещё одна сессия», а это - на другой
    // вопрос: «шевелится ли сервер вообще». Их легко перепутать, и они
    // расходятся: памяти может быть вдоволь, а машина стоять колом,
    // потому что все ядра заняты. На сервере с двумя ядрами это ближе,
    // чем кажется - десяток сессий и боты живут именно там.
    //
    // Число ядер обязательно рядом: «загрузка 2,4» сама по себе не
    // значит ничего. 2,4 на восьми ядрах - треть машины, 2,4 на двух -
    // очередь из ждущих процессов. Отношение и есть ответ.
    //
    // Берётся среднее за МИНУТУ, а не за пять: трей отвечает на «что
    // происходит сейчас», и пятиминутное там запаздывало бы ровно
    // на столько, на сколько обычно длится всплеск.
    var load1: Double = 0    // 0 - не измеряли
    var cores = 0

    var isEmpty: Bool { return totalMB == 0 && cores == 0 }

    // «ОЗУ занято 2,6 из 3,8 ГБ · своп 1,7 из 6,9 ГБ · загрузка 2,4 из 2».
    //
    // Заполняется ТОЛЬКО с удалённой машины: там и бывают сложности,
    // а на маке памяти обычно вдоволь. Ветка без availableMB осталась
    ...
    var text: String {
        ...
        if !loadText.isEmpty {
            s += s.isEmpty ? loadText : " \u{00B7} " + loadText
        }
        return s
    }
    ...
    // число загрузки не значит ничего, и показывать его одно было бы
    // хуже, чем не показывать вовсе.
    //
    // Слово короткое намеренно. Эта строка дописывается к строке памяти,
    // а та уже сама по себе самая широкая в меню - «загрузка 2,4 из 2»
    // добавляло к ней столько, что меню раздвигалось под неё. Рост
    // ширины от содержимого закрывали в 1.6.2, и заводить его заново
    // ради шести букв нельзя.
    var loadText: String {
        guard cores > 0 else { return "" }
        let n = String(format: "%.1f", load1)
        return L("ЦП \(n.replacingOccurrences(of: ".", with: ",")) из \(cores)",
                 "CPU \(n) of \(cores)")
    }

    // Машина упёрлась в процессор: ждущих процессов больше, чем ядер.
    //
    // Порог ровно в одно ядро, без запаса: loadavg в линуксе считает
    // и работающие, и ждущие диск, и держать «безопасную» прибавку
    // значит молчать ровно там, где очередь уже выстроилась.
    var isOverloaded: Bool {
        return cores > 0 && load1 > Double(cores)
    }
}
...
enum Sessions {
    ...
    static func lines(_ list: [AgentSession], nameLimit: Int = nameLimit,
        ...
        for host in orderedNames {
            ...
            //
            // Слово Антона 17.09.2026: «на маке не нужно именно траты
            // чужих ресурсов показывать, тут ведь много рабочего и так
            // происходит; мне важно сторонние сервисы контролировать».
            // На маке эта строка отвечала бы «браузер и редактор заняли
            // 10 ГБ» - правда, которая ничего не решает: он их и так
