...
for name in names:
    ...
    # Ведут ли этот разговор через Remote Control - с телефона или из
    # браузера. Едет «да/нет», а НЕ сам `bridgeSessionId`: на экран он не
    # попадает, а это ключ к чужому разговору на стороне Anthropic.
    #
    # Без этой строки признак терялся бы ровно там, где он нужен: на
    # сервере и моноблоке через мост идёт каждая сессия, на маке - ни одной.
    item["bridged"] = bool(rec.get("bridgeSessionId"))
